...
The runbook for a session on the Windows/WSL2 training box (the machine map is in the
root `README.md`). It overrides nothing: `3d/CLAUDE.md` and `rl/CLAUDE.md` still say what
is source and what is output. It only says which of their steps this machine can actually
run, and what it costs to find out the hard way.
...
The servo's measured no-load speed is 3.86 rad/s and `actuator.py`'s law still frees to
5.9 (PLAN.md 3c): read a policy's p95 joint speed against 3.86 with `tools/ceiling.py`
before believing its distance.

`rl/uv.lock` is versioned on purpose — a pinned resolution is the point when three
machines share one tree. If `uv sync` changes it, commit that on its own.
...
The baseline distances in `3d/CLAUDE.md` were measured **on the mac**. MuJoCo is
deterministic for a given model, seed and version, but not necessarily across platforms —
and the flat trot sits near a bifurcation where 11 g moves it 180 mm. So the mac's numbers
are not a baseline here. Run the *unchanged* tree on this box first and compare against that.
