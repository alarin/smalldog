...
| `lidar_tilt.py` | `LIDAR_TILT` sweep off `out/sim/mini_dog.xml`, robot standing: per frame, the floor strip ahead of the front feet, the near edge, returns on a step / box / table leg, rays with no return, the deck-lip margin. The site is re-posed, the CAD is not rebuilt |
