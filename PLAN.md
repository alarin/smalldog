...
wall-clock. `keys[4]` in `_sample_episode` is left spare rather than renumbered, because
renumbering changes every other draw for a given seed and would make runs incomparable.
`check_model.py` checks the field on the compiled training model (it must run on the
robot, where there is no jax); `python -m env.randomize` is the batched-shape probe on
the jax side, written because `mu_load`'s only symptom was ever a shape.
