...
def _selftest(dt_int=1e-4) -> int:
    ...
    weak = {
        "mu_load": "the bidirectional ladder measures it on HARDWARE (0.25-0.30 "
                   ...
                   "HALF FIXED 2026-09-11: simulate() now uses Karnopp rather "
                   "than tanh(w/v_eps), so the bench model STICKS - below v_eps "
                   "friction opposes the net applied torque up to "
                   "tau_c + mu_load*|tau_t| instead of fading to nothing at "
                   "rest. A sub-breakaway load now comes to rest where it used "
                   "to creep at 0.013 rad/s (actuator.py --selftest probes "
                   "both). What is NOT fixed is friction()/motor_torque(), "
                   "which rl/env/walk.py and rl/eval.py call with MuJoCo owning "
                   "the load: they cannot see the net torque, so stick there "
                   "has to be MuJoCo frictionloss - PLAN.md step 2b. NOTE this "
                   "changed the model the fit runs against, so st3215.json "
                   "predates it and a refit is due",
